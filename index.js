...
function is31Flavors(copyOriginal){
...
function addFlavor(copyOriginal, flavorTown){
  copyOriginal.unshift(flavorTown);
  return copyOriginal;
}
...
function removeLastFlavor(copyOriginal){
  copyOriginal.pop();
  return copyOriginal;
}
...
function getFlavorByIndex(copyOriginal, num){
  return copyOriginal[num];
...
function removeFlavorByName(copyOriginal, string){
  copyOriginal.splice(copyOriginal.indexOf(string),1);
  return copyOriginal;
...
function filterByWord(copyOriginal, word){
  const newArr;
  for (copyOriginal.length) {
    if (copyOriginal[index].includes(word)) {
      newArr.push(word)
    }
  }
}
